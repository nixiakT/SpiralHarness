...
| Skills | Immutable declarative packages, direct generated revisions, fixed rules disclosure, request materialization, settled request-inclusion replay, and one score-free process-local matched plumbing run; replay still requires the original capability/live ledger writers, and there is no provider/runtime-reset attestation, claim evidence, or promotion authority | Run a new preregistered scored matched claim-capture study with independent provider-delivery, runtime-activation, adherence, behavior, revert, and placebo evidence before skill search. |
