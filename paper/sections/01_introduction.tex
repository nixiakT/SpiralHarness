...
enough evidence to \emph{commit} an adaptively selected harness edit?
\end{quote}

Our answer is \systemname, a protocol whose target architecture separates a
mutable evolution plane from an independent verification plane and a one-time sealed authority
...
controlled-fault \(2\times2\) factorial prospectively estimates feedback-guidance,
promotion-verification, and interaction effects under its randomized design.  The central empirical claim
is an intersection rather than a score-only win: the joint \fullarm{} policy
must control and reduce family-level false promotion, improve sealed utility
relative to \scorearm{} and the unmodified harness, and pass protected-behavior
and cost non-inferiority.  The usefulness requirements prevent a conservative
reject-all policy from appearing safe.

\paragraph{Contributions.}
This draft makes three contributions, with empirical claims deliberately kept
separate from design claims:
\begin{itemize}
  \item A formal commit decision for adaptive harness search, with immutable
  typed edits, an explicit trusted surface, and family-level false promotion as
  a co-primary endpoint rather than a diagnostic afterthought.
  \item A randomized mechanism gate using child, parent, exact-revert, and
  matched-placebo arms, campaign-level error control, and distinct utility,
  protected-behavior, and resource constraints.
  \item A prospective same-model study that joins a controlled-fault
  feedback-by-promotion factorial to one-time sealed real-task evaluation.  Its
  protocol and conjunctive conclusion are
  \evidenceblocked{pending preregistration, confirmatory independent search
  replications, and atomic sealed release}.
\end{itemize}

We do not claim the first self-improving harness, activation check, statistical
gate, held-out acceptance test, or use of execution evidence.  The narrower
prospective distinction is a decision rule for credible adaptive commits:
child--parent--revert--placebo attribution along an
activation--adherence--behavior chain, primary family-level false-promotion
control, and factorial separation of feedback from promotion authority.
