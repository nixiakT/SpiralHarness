...
only matched aggregate scores, intervals, resource totals, and decisions.  The
solver, proposer, mutation grammar, task coordinates, and budget are otherwise
held fixed.  The central empirical question is whether \fullarm{} reduces
false promotion and improves sealed utility relative to \scorearm{}.
...
\begin{itemize}
  ...
  \item HarnessFaultBench, a factorial controlled benchmark whose oracle causal
  surface permits measurement of localization, verified repair, and false
  promotion rather than score alone.
  \item A preregistered same-model, budget-matched study across harness surfaces
  and real tasks.  Its main conclusion is
  \evidenceblocked{pending independent search replications and atomic sealed
  release}.
\end{itemize}

We do not claim the first self-improving harness, the first held-out acceptance
test, or the first use of execution evidence.  The proposed distinction is the
combination of intervention-linked mechanism evidence, independent promotion,
and campaign-level measurement of false promotion.
